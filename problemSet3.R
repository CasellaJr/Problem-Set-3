...
library(cluster)
fviz_dend(hclust(daisy(s), method="ward.D"),cex=0.5, main="Ward linkage method and Gower distance")
cor(cophenetic(hclust(daisy(s),method="ward.D")), daisy(s))
fviz_dend(hclust(daisy(s), method="ward.D2"),cex=0.5, main="Ward D2 linkage method and Gower distance")
cor(cophenetic(hclust(daisy(s),method="ward.D2")), daisy(s))
...
In this case the Ward D2 linkage method and the Gower’s distance produces the highest value of this statistic: 0.9140627.'
...
hc_labels
table(hc_labels)
#DA CONTROLLARE: V. De Geneve dovrebbe essere clusterizzato con altre città svizzere. Poi controllare su internet
